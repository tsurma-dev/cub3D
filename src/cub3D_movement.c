...
void	calculate_positions(t_map *map, t_player_movement *movement,
		int sideways)
{
	movement->forward_px = map->px + map->pdx * movement->f_move_speed;
	movement->forward_py = map->py + map->pdy * movement->f_move_speed;
	movement->sdways_px = map->px + map->plane_x * movement->lat_move_speed
		* sideways;
	movement->sdways_py = map->py + map->plane_y * movement->lat_move_speed
		* sideways;
...
void	update_player_position(t_map *map, int d, int sideways)
{
	...
	if (map->mapp[movement.forward_map_y * map->mapx
			+ movement.forward_map_x] == 1)
	{
		if (map->mapp[movement.sdways_map_y * map->mapx
				+ movement.sdways_map_x] != 1)
		{
	...
	else
	{
		...
		if (sideways != 0 && map->mapp[movement.sdways_map_y * map->mapx
				+ movement.sdways_map_x] != 1)
		{
			map->px = movement.sdways_px;
			map->py = movement.sdways_py;
